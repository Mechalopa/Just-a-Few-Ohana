...
public class ModEventBusSubscriber
{
	...
	{
//		ModPlacements.FLOWER_PLAIN_FEATURE = BuiltinRegistries.register(BuiltinRegistries.PLACED_FEATURE, new ResourceLocation(JAFOhana.MODID, "flower_plain"), Feature.FLOWER, new RandomPatchConfiguration(64, 6, 2, PlacementUtils.onlyWhenEmpty(Feature.SIMPLE_BLOCK, new SimpleBlockConfiguration(BlockStateProvider.simple(Blocks.BLUE_ORCHID)))));
//		ModPlacements.FLOWER_PLAIN_FEATURE = Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation(JAFOhana.MODID, "flower_plain"), Feature.FLOWER.configured((new BlockClusterFeatureConfig.Builder((new WeightedBlockStateProvider()).add(ModBlocks.DAYFLOWER.get().defaultBlockState(), 1).add(ModBlocks.EVENING_PRIMROSE.get().defaultBlockState(), 1).add(ModBlocks.MILK_VETCH.get().defaultBlockState(), 1), SimpleBlockPlacer.INSTANCE)).tries(64).build()).decorated(Features.Placements.ADD_32).decorated(Features.Placements.HEIGHTMAP_SQUARE));
...
//		ModPlacements.FLOWER_PLAIN = PlacementUtils.register("flower_plain", VegetationFeatures.FLOWER_SWAMP, RarityFilter.onAverageOnceEvery(32), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP, BiomeFilter.biome());
	}
